
# 3-nomad-again #
![3-nomad-again.png](3-nomad-again.png)
- large map with sparce resources
- resources are stack (like 10 tress on one square) to limit the number of villagers you can gather with
- buildings are cheap, quick to build, build units quickly, week, and slowly die
- you start with 2 bands of villagers
# 3-nomad-conquest-only-farms #
![3-nomad-conquest-only-farms.png](3-nomad-conquest-only-farms.png)
- you can't build farms but there are a lot on the map
- villagers start scattered 
# 4p-map-control #
![4p-map-control.png](4p-map-control.png)
# 4p-really-big-map #
![4p-really-big-map.png](4p-really-big-map.png)
- wow this map is big
- pretty empty as well, you base will need to be spread out
# 5 sheep #
![5 sheep.png](5-sheep.png)
- sheep spawn in the middle
- bring n sheep home to win
# 5p-rotating-wonder-3 #
![5p-rotating-wonder-3.png](5p-rotating-wonder-3.png)
- players take turns defending a wonder
# 550 #
![550.png](550.png)
- 2v2
- one player on each side has 500 pop and is stuck in the dark age
- one player on each side has 50 pop
# bb #
![bb.png](bb.png)
# dock flower #
![dock flower.png](dock-flower.png)
- docks are control point 
- control enough docks and get an endless supply of units
# docks-2-1 #
![docks-2-1.png](docks-2-1.png)
# inertia_tiny2_v005 #
![inertia_tiny2_v005.png](inertia_tiny2_v005.png)
- destory buildings for money
- control obelisks for income 
# llama 2 #
![llama 2.png](llama-2.png)
- control llamas for income in a bit
# p2-war-3 #
![p2-war-3.png](p2-war-3.png)
- destroy 2 enemy castles to win 
- designed to be played co-op
# p3-docks #
![p3-docks.png](p3-docks.png)
- docks are control points
- pop cap 50
- villagers start scattered
- you can build town centers in the dark age
# p3-normal-1-1 #
![p3-normal-1-1.png](p3-normal-1-1.png)
# p4_collect_carts #
![p4_collect_carts.png](p4_collect_carts.png)
- destory endlessly spawning buildings to get carts
- collect n carts, get a free wonder
- off-map ram powered unit production
# p4_x_005 #
![p4_x_005.png](p4_x_005.png)
- get a relic cart to your corner to score
- off-map ram powered unit production
- off-map economy 
# p4-another-another-map-control-4 #
![p4-another-another-map-control-4.png](p4-another-another-map-control-4.png)
- safe economy
- sheep control points, control enough for long enough to win
# p4-another-map-control #
![p4-another-map-control.png](p4-another-map-control.png)
- safe economy
- control points
# p4-collect-collect-carts-4 #
![p4-collect-collect-carts-4.png](p4-collect-collect-carts-4.png)
- destory endlessly spawning buildings to get carts
- collect n carts, get a free wonder
- off-map ram powered unit production
- economy ??
# p4-rams-llama #
![p4-rams-llama.png](p4-rams-llama.png)
- off-map ram powered unit production
- control points give money(?)
- collect relic cars to win(?)
# p4-rams-llama-3 #
![p4-rams-llama-3.png](p4-rams-llama-3.png)
- off-map ram powered unit production
- control points give money(?)
- collect relic cars to win(?)
# sheep-wars-4-3 #
![sheep-wars-4-3.png](sheep-wars-4-3.png)
- sheep spawn in the middle
- collection 100 sheep to get an unlimited supply of teutonic knights
# vills-appear #
![vills-appear.png](vills-appear.png)
- you can't build villagers
- villagers appear randomly around the map
# waves-2 #
![waves-2.png](waves-2.png)
- played in rounds
- units build very quickly
- no economy 
- merchants spawn every round
- kill merchants to win
# waves-3 #
![waves-3.png](waves-3.png)
- played in rounds
- 2v2
- has an arena you can telliport unit to
- kill the other teams city wall to win the round
# x_v001 #
![x_v001.png](x_v001.png)
- you have 3 ram that each spawn a single unit type
- the unit types depend on your side
- cross the lake and return to win
- 3 player
# x_v003 #
![x_v003.png](x_v003.png)
- you have 3 ram that each spawn a single unit type
- the unit types depend on your side
- cross the lake and return to win
- 8 player